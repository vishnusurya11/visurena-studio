"""Time arithmetic the cue plan and the shot grammar share.

The music's measured spans ARE the shot list (`studio.cue_plan`); this module
keeps only the primitives those spans are quantised and floored with:

  * `MIN_SHOT` -- the one floor under any shot, the grammar's smallest size;
  * `quantise` / `snap` -- frame rounding and pulling a moment onto a grid;
  * `cut_points` / `lengths_of` -- the corpus-arc walk over an onset grid,
    kept for the grammar's own tests of the floor and the ceiling.

History: until row 55 of the trailer BUILD this module carried "the beat
walk" -- `plan_cuts` with its acts, figure and beat lock -- which INVENTED a
cut list and then hoped the music had an event near each cut.  Run 12 onward
reads the cuts off `CuePlan.spans`, so nothing downstream may synthesise one.
...
this was its own 0.4 the planner emitted a 0.417 s shot at 128 BPM and the
...
def cut_points(duration: float, grid: list[float], start: float = 0.0,
    ...
    NOT the trailer's path: the trailer cuts on `CuePlan.spans`, which are
    measured off the rendered cue and invent nothing.  This survives for the
    shot grammar's tests of the one floor and the one ceiling, and for any
    onset-list cut (a song video) that has no cue plan to read.
    """
