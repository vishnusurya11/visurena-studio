...
def climb(ladder: Ladder, step: str, attempt: Callable[[Rung, int], Any], gate: Gate,
          budget: Budget, learn: Callable[[Learning], None], gate_name: str = "gate") -> Outcome:
    """Try, gate, climb; go terminal when rungs or time run out."""
    taken: list[str] = []
    for n, (rung, i) in enumerate(_tries(ladder)):
        if n > 0 and not budget.can_afford(step, rung.cost_seconds):
            learn(Learning(step=step, gate="budget", measured=budget.remaining(step),
                           threshold=rung.cost_seconds, action=ladder.terminal,
                           attempt=n, terminal=True))
        ...
        learn(Learning(step=step, gate=gate_name, measured=measured, threshold=threshold,
                       action=rung.name, attempt=n + 1))
    learn(Learning(step=step, gate=gate_name, action=ladder.terminal,
