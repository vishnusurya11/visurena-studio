...
| 48 | the cue is edited, never stretched: `slice_bars`, `splice` (equal-power, beat/2 or 10 ms on a hit), `splice_out`, `repeat_bars`, `hole`, `stop_at`, `section_gains`, `compatible`, `conform`; `conform` rung in step 03 before reauthor | `studio/cue_edit.py` | `test_splicing_out_a_bar_shortens_the_cue_by_a_bar_and_keeps_the_downbeats_on_the_grid`, `test_a_join_on_a_hit_is_ten_milliseconds` | [ ] |
