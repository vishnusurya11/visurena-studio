...
| 66 | WHAT THE CUE SUPPLIES THE EDITOR -- the dimension no row before this one measured, and the one the owner has been naming for three trailers. A cut point must be an ATTACK (`trailer_edit` picks its cuts from `beatmap.onsets`) and no shot may run past `MAX_SHOT` 4.0 s, so a cue that leaves a longer wait than that inside its own material forces the editor to hold a shot past the ceiling or cut on silence. MEASURED, run 19: cue-1002 supplied 19 attacks over 113.8 s -- 0.86 per shot against the 1.5 `beatmap.GRID_DB` was tuned to yield -- with a 14.6 s wait, and plan.json duly holds a 17.51 s span in a 106 s trailer; 1001 read 1.29/shot and 5.9 s, 1004 0.64/shot and 14.5 s. EVERY seed starved and every one passed the old gate on level alone (ask 1.00, form 2/2, fit 0.8 dB). `cue_supply` reads attacks, material (asked dropouts removed), rests (bars ridden LOW -- `cue_qc.long_shots_on_holds` already says a long shot belongs over a trough -- and the title's hold to the stop) and the run-out after the last attack, which is the picture ARRIVING; step 03 measures it per seed (`supply_of`), RANKS on it ahead of the ride fit (`SUPPLY_BAND`), GATES on it (`verdict`), and the re-ask now names the wait in the player's terms (`waited`: something struck at least every 4.0 s, through the quiet sections too). SECOND FINDING, from the same instrument: ARC_VERSION 9's own voicing was burying the attacks. The reference LTAS is finished music whose transients are already in it; our render's bed and its strokes sit in different bands, so the match boosts the bed +6 where the cut needs the stroke -- a 120 BPM stroke over a 110 Hz bed went from 30 cut points to 0 and its envelope spread from 7.3 dB to 2.3. `cue_voicing.voice` now measures its own output and halves the match until `KEEP_ATTACKS` 0.9 of the cut points survive (raw-1002: 22 -> 20, was 22 -> 17 at full limit). The spectrum is worth having; a spectrum nobody can cut to is not | `studio/cue_supply.py`, `studio/cue_voicing.py`, `scripts/trailer/step_03_music.py` | `tests/test_cue_supply.py` x10, `tests/test_cue_voicing.py` (`test_voice_backs_off_before_it_buries_the_attacks`, `test_voice_still_matches_when_the_attacks_survive_it`), `tests/test_step_03_music.py` (`test_supply_of_reads_the_cut_points_the_cue_offers_the_editor`, `test_best_of_prefers_the_seed_that_feeds_the_editor_over_the_fitter_one`, `test_a_cue_that_starves_the_editor_fails_the_verdict`, `test_the_title_cards_own_hold_is_not_the_editor_waiting`, `test_the_reauthor_tells_the_model_how_long_the_picture_waited`) | [x] |
| 67 | THE TRAILER SPEAKS: the join between a slate and a voice is an ID, and nothing checked it. MEASURED, run 19: 4 of 5 lines spoken, and the missing one was the `stakes` line whose speaker was "Police Inspector" -- a character cue written for a reader, against 23 cast cards filed under snake_case ids -- while four castable `stakes` spares sat unused in the same lines.json. Two pools, two conventions: `quote_pool` and `source_pool` key on the analysis id, `screenplay_pool` took the screenplay's cue verbatim, and the fixture never caught it because its screenplay used ids AS cues. `trailer_story.speaker_id` now resolves a cue through the registry alias index (Dr. Watson -> john_watson) and an unresolved cue enters as a CARD, the way narration already does, so the slate knows it is choosing one; on the real book 907 of 911 speakers resolve, the four that do not are registry ids the analysis never carded. `step_05_voice` stops carding a line for want of a voice: `next_castable` draws the next unused line of the same FUNCTION whose speaker the book can card, `reference_for` designs that voice on demand, and the learning is `spare_drawn` rather than a terminal refusal. The rule: when one step's output is another step's input, the identifier that joins them is a contract -- test it with a fixture that does NOT already speak the destination's dialect | `studio/trailer_story.py`, `scripts/trailer/step_05_voice.py` | `tests/test_line_pools.py` (`test_a_screenplay_cue_is_resolved_to_the_cast_id_it_names`, `test_a_cue_naming_nobody_in_the_cast_is_a_card`), `tests/test_step_05_voice.py` (`test_a_speaker_with_no_cast_card_draws_a_spare_of_the_same_function`, `test_a_speaker_with_no_cast_card_and_no_spare_is_still_a_card`) | [x] |
| 68 | THE TRAILER GETS A SCREENPLAY, and everything is built FROM it. Six research passes settled the question the pipeline never asked: a theatrical trailer has no screenplay because its picture is SEARCHED FOR in footage that already exists ("you wouldn't want to write a script calling for scenes and settings that aren't in the negative" -- Fred Greene, entertainment copywriter, UCLA TFT), while every domain whose footage must be COMMISSIONED writes the plan first: teasers before principal photography, game trailers, and book trailers, our own vertical. We render every shot, so the picture column is ours. The order is now the professional one and the reverse of what this repo did -- page, then the cue asked FOR the page, then the picture cut TO it, then the bed STOPPING where it speaks. `trailer_draft` writes the page from the book and refuses a line the book never wrote or a speaker with no cast card; `trailer_shots` writes 18 Ref2V documents and refuses a sheet that does not exist; `script_cue` reads 14 landmarks off the page's own sound direction; `script_qc` measures the MASTER against the page, which two research passes found no published system does. Format set by the owner: 60 s +-10, 9:16, title named early and again on the hit, closing on the reopened question then the next action. First cut: 53.5 s, acts 28/39/33, speech 48%, 9 of 9 lines spoken -- against run 19's 106 s, act one at 1%, speech 11.5%, 4 of 5 spoken | `studio/trailer_script.py`, `trailer_draft.py`, `trailer_shots.py`, `script_cut.py`, `script_cue.py`, `script_mix.py`, `script_qc.py`, `scripts/trailer/render_script.py` | `tests/test_trailer_script.py`, `test_trailer_draft.py`, `test_trailer_shots.py`, `test_script_cut.py`, `test_script_cue.py`, `test_script_mix.py`, `test_script_qc.py`, `test_render_script.py` | [x] |
| 69 | SAY WHAT THE SHOT IS OF. The first script-driven render drew CANDLES for a beat whose line is "I will now cut one of these pills in two", and the reason was measurable: B08's prompt ran 1009 words and said "pills" TWICE. 303 words were byte-identical across all eighteen prompts (generic light, parallax, materials, performance, continuity) and ~150 more re-described Holmes' face in prose while the reference IMAGE was already carrying it. B05 contradicted itself outright -- "the shot holds an EMPTY 3 Lauriston Gardens" followed by "a gloved hand lifting a pill", and "the shadowed side of the FACE keeps visible detail" in a shot with no face -- so the model rendered what the document emphasised, which was the boilerplate. `shot_document` states the SUBJECT first and last, keeps each craft line to one sentence, and never asserts what the shot contradicts: template overhead 303 -> 120 words, whole document 1009 -> 370, shared text 303 -> 70. Every shot now names the one thing it is of, and a beat written as a montage is refused because the renderer cannot cut. Re-rendered B08: the pills are pills. Also written into both briefs: H3 keeps TWO faces, so a beat naming three or four had two invented, and an invented face is a different person in every shot | `studio/shot_document.py`, `trailer_shots.py`, `trailer_draft.py` | `tests/test_shot_document.py` x9 (`test_the_subject_is_named_first_and_named_again_at_the_close`, `test_a_shot_with_a_hand_in_it_is_never_called_empty`, `test_a_faceless_shot_does_not_light_a_face`, `test_the_template_costs_little_of_the_document`) | [x] |
| 70 | ONE VOICE, SEVERAL FEELINGS. `voice.clone_line` had one reference per character and no emotional control, so Holmes stating a deduction and Hope naming the hour of his revenge read identically -- and a trailer lives on that difference. `voice_bank` renders one clip per character per emotion through Qwen3-TTS VoiceDesign (free-text instruct, no fixed emotion list); `voice_say` hands IndexTTS2 the TIMBRE in `ref_audio` and the FEELING in `emotion_audio`, which is what that node takes them apart for. The bank is ONE baseline passage for every character, so the voice is the only variable, and every emotion opens on the same sentence, so the feeling is the only variable in the other direction -- which is what a one-shot engine replicates onto a new line. Emotions chosen for the JOB (calm, curious, cold, urgent, grieving, threatening) rather than for IndexTTS2's slider names: `happy` is useless in a mystery and `curious` is the register the book gives Holmes, and neither engine constrains us since the reference route takes any clip. MEASURED, 18 clips: same words, Holmes reads cold at 4.4/4.1/4.6 s and curious at 2.7/2.0/2.3 s -- twice the length from the delivery note alone. THREE THINGS THE OWNER'S QUESTIONS CAUGHT: every character was reading the same voice-demo script with the name swapped; the text said the emotion out loud ("how I sound when I am threatening" is the actor reading the stage direction); and the delivery notes were three-word LABELS, which give an engine nothing to act on -- they are 30-60 words of pace, pitch, breath and attack now. The PAGE names how each line is said: guessing it from the beat's function gave Hope's revenge declaration the grieving read and three deductions urgent, because "escalation" contains "escalat". And the bed's stop window follows the LINE, not the beat: B02's beat is 3.0 s and its cold delivery runs 4.41 s | `studio/voice_bank.py`, `voice_say.py`, `trailer_script.py`, `script_mix.py` | `tests/test_voice_bank.py` x16, `test_voice_say.py` x3, `test_script_mix.py` (`TestTheWindowFollowsTheLine`) | [x] |
