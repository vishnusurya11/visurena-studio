...
| 58 | the trailer's genre and mix are the caption's CONSTANTS (`TRAILER_GENRE`, `TRAILER_MIX`, written by `head()` into the first sentence and the Sonics line); the book's `genre`/`mix_space`/`era_reference` are colour on them. Sixteen cues over runs 9-12 opened on "a small acoustic ensemble of 1881 London" and the user rejected the sound twice; §1's own table row asked for that ensemble by name | `studio/music_tone.py`, Scarlet `tone.json` v3 | `TestTrailerConstants` ×4 | [x] |
| 59 | a LISTEN gate: `cue_listen.listen` ranks the caption's brief against the sounds a cue has come back as (parlour, song, drone) in CLAP's text-audio space; rank, never a threshold; verdict written beside the cue. Runs 9-12 graded form and grid and could not hear that all sixteen cues were parlour pieces. Contract and fake-embedder tests shipped; the real `ClapEmbedder` (transformers, lazy) is unvalidated until it hears the sixteen rejected cues as parlour and a run-13 cue as the brief, then it wires into step 03 | `studio/cue_listen.py` | `tests/test_cue_listen.py` ×9 | [ ] |
