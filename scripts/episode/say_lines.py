...
def render(book: Path, episode: Episode, line: Line, out_dir: Path, attempt: int) -> dict:
    """One line's file and its measured seconds; nothing listened to yet."""
    out = out_dir / f"l{line.index:02d}.wav"
    reference = reference_for(book, line.speaker)
    ...
    return {"index": line.index, "speaker": line.speaker, "kind": line.kind, "shot": line.shot,
            ...
            "seed": said.seed, "tries": attempt + 1}
...
def main(book_id: str, number: int) -> None:
    ...
    for attempt in range(TRIES):
        for line in todo:
            ...
            records[line.index] = render(book, episode, line, out_dir, attempt)
