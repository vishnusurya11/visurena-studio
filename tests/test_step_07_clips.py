...
A take whose face does not read as its reference climbs seed x2 then a
whole-take close-up; the terminal rung ships the BEST take capped at a
short shot.  A beat is dropped only when no take exists at all -- the
budget gone before its first render, or the render itself failing.
Renders and recognisers are faked; nothing here touches a GPU.
"""
...
def rendered(monkeypatch):
    """Faked renders and recognisers: `scores` is consumed per bound take."""
    calls = []
    ...
    def render_take(values, bound, refs, book, dest):
        ...
        return dest
    state = {"calls": calls, "scores": [], "clock": [0.0]}
    ...
    monkeypatch.setattr(step, "embed_file", lambda s, p: (np.array([1.0, 0.0]), 90))
    monkeypatch.setattr(step, "clip_identity",
                        lambda s, v, sec, r, w: state["scores"].pop(0))
    return state
...
class TestRun:
    ...
    def test_a_weak_face_rerolls_the_seed_then_binds(self, ctx, rendered):
        rendered["scores"] = [(0.2, 90), (0.5, 90)]
        step.run(ctx.codex_id, ctx)
        clip = next(c for c in clips_doc(ctx)["clips"] if c["beat_id"] == "B00")
        assert clip["similarity"] == pytest.approx(0.5) and clip["capped"] is None
        rows = load(ctx.learnings_path)
        assert [r.action for r in rows] == ["reroll_seed"]
        assert rows[0].measured == pytest.approx(0.2) and rows[0].threshold == step.BOUND_AT
    ...
    def test_a_face_that_never_binds_ships_its_best_take_short(self, ctx, rendered):
        rendered["scores"] = [(0.2, 90), (0.3, 90), (0.25, 90)]
        step.run(ctx.codex_id, ctx)
        doc = clips_doc(ctx)
        clip = next(c for c in doc["clips"] if c["beat_id"] == "B00")
        assert clip["capped"] == step.SHORT_SHOT and clip["similarity"] == pytest.approx(0.3)
        assert doc["dropped"] == []
    ...
    def test_a_face_too_small_to_read_is_accepted_and_flagged(self, ctx, rendered):
        rendered["scores"] = [(0.1, 30)]
        step.run(ctx.codex_id, ctx)
        clip = next(c for c in clips_doc(ctx)["clips"] if c["beat_id"] == "B00")
        assert clip["capped"] is None and clip["face_px"] == 30
